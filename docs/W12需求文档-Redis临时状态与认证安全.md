...
> 当前唯一能力点：**W12-2 Redis 并发失败计数精确性（R12-2-11，代码已实现并通过新鲜验证，掌握确认待完成）**
...
| W12-2 | 失败登录限流 | Auth Service Redis 客户端边界、限流模块、HTTP 行为与测试 | 进行中：Redis client、限流模块、`app.js`、`server.js` 和真实 HTTP/Redis 基础/阈值/用户隔离/IP/TTL/未知账号/非法输入测试已验证；R12-2-11 并发合同待完成掌握确认 |
...
- [ ] 用户能独立解释 Redis 原子计数、`EXPIRE ... NX` 和有界过期轮询的证据关系；
...
- **当前能力点**：W12-2 Redis 并发失败计数精确性（R12-2-11）已实现并通过新鲜验证，掌握确认待完成；
...
- **下一待办项**：完成 R12-2-11 复盘问答；通过前不进入 Redis 故障、JWT 撤销或 Docker。
