...
> 当前唯一能力点：**W12-2 真实 HTTP 的 username 阈值 429 与 bcrypt 短路验证（R12-2-01～03，代码已实现并通过新鲜验证，掌握确认待完成）**
> 当前授权：`app.js` 的依赖校验、`/register`、`/me` 和真实 Redis 合同测试已按用户授权代写并完成掌握确认；本轮进入临时数据库与 HTTP 联调设计，仍不提交 Git。
...
| W12-2 | 失败登录限流 | Auth Service Redis 客户端边界、限流模块、HTTP 行为与测试 | 进行中：Redis client、限流模块、`app.js` 和 `server.js` 启动编排已实现且掌握确认通过；真实 HTTP 联调与自动化测试已通过，掌握确认待完成 |
...
- [ ] 用户能独立解释阈值次与已受限次的调用差异、真实 bcrypt 包装器与 Redis 最终状态；
...
- **当前能力点**：W12-2 真实 HTTP 的 username 阈值 429 与 bcrypt 短路验证（R12-2-01～03）已实现并通过新鲜验证，掌握确认待完成；
...
- **当前未产生的证据**：本轮用户掌握确认尚未完成；IP 跨账号、TTL 到期、Redis 故障映射与 JWT 撤销也未完成。
- **下一待办项**：完成 R12-2-01～03 的复盘问答；通过前不进入 IP 跨账号、TTL 到期、Redis 故障、JWT 撤销或 Docker。
