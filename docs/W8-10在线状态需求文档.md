...
> 状态：已制定，尚未开始编码。后续 W8 的在线用户列表、上线与下线通知均以本文档为准。
...
- 离线消息、数据库持久化、跨重连补发；
- 多端同时登录；本版本采用**一个用户名只保留一个活动会话**的策略，重复登录的具体握手处理必须在实现前单独设计并验收，不能默默覆盖映射；
...
客户端的唯一状态是 `QStringList m_onlineUsers`（或语义等价的私有容器），来源只能是服务端快照；`QListWidget` 只是显示层，不得成为状态来源。`m_conversations` 仍只保存真实聊天消息，不能混入在线用户名或空会话。
...
**重复登录前置条件：** 现有 `emplace()` 不能静默处理同名第二会话。开始服务端编码前，必须先选择并落实“拒绝第二连接”或“新连接顶替并安全关闭旧连接”其中之一；在该策略完成前，双客户端验收使用不同用户名。
...
ChatClient 不保存在线名单、不直接操作 `QListWidget`，以保持网络层与界面层分离。
...
3. **服务端广播时机：** 接入认证成功与认证用户断开两个边界；确保先改映射再广播，且未认证断开不广播。
4. **客户端解析与信号：** 增加 type=8 分派、严格 JSON 校验和 `onlineUsersChanged`；添加有效/非法正文测试或可重复的手工注入验证。
5. **Qt 列表显示：** 新增控件、私有模型、刷新函数、断开清理与点击回填；不改变现有会话模型。
6. **端到端验收：** 使用 Alice/Bob/Carol 分别验证首次认证、后续上线、断线下线、自己过滤、点击回填、私聊回归；最后才讨论重复登录策略。
...
| 同名重复登录 | 按已选策略明确成功/失败；不得出现双向映射不一致或旧会话断开误删新映射 |

## 10. 完成定义与后续边界

完成本功能必须满足：构建成功、现有 CTest 全绿、上述正常与断线场景人工验收通过，且协议非法正文不会破坏已有 UI 状态。

本功能完成后，W8 的“在线用户列表 / 上线 / 下线通知”主项结束。重复登录策略若未随本轮完成，必须在路线中显式保留为稳定性补强，不能默认其已正确处理。
