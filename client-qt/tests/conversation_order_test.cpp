...
ChatMessage makeReceivedMessage(const QString& message_id, const QString& local_id,
                                const QString& content, const qint64 server_received_at_ms)
{
    ChatMessage message;
    ...
    message.send_at = QDateTime::fromString(QStringLiteral("2026-08-17T10:00:00.000Z"),
                                             Qt::ISODate);
...
bool testHistoryAfterRealtimeKeepsAscendingOrder()
{
    ...
    const ChatMessage message_3 = makeReceivedMessage(QStringLiteral("message-3"),
                                                       QStringLiteral("local-3"),
                                                       QStringLiteral("M3"), 300);
    client.chatMessageReceived(message_3);

    const QList<ChatMessage> history_messages{
        makeReceivedMessage(QStringLiteral("message-1"), QStringLiteral("local-1"),
                            QStringLiteral("M1"), 100),
        makeReceivedMessage(QStringLiteral("message-2"), QStringLiteral("local-2"),
                            QStringLiteral("M2"), 200),
        message_3
    };
...
bool testAcceptedMessageReordersLocalPendingMessage()
{
    ...
    client.chatMessageReceived(makeReceivedMessage(QStringLiteral("message-2"),
...
bool testSameTimestampUsesMessageIdAsTieBreaker()
{
    ...
    client.chatMessageReceived(makeReceivedMessage(QStringLiteral("message-b"),
                                                    QStringLiteral("local-b"),
                                                    QStringLiteral("B"), 300));
    client.chatMessageReceived(makeReceivedMessage(QStringLiteral("message-a"),
                                                    QStringLiteral("local-a"),
                                                    QStringLiteral("A"), 300));
...
int main(int argc, char* argv[])
{
    ...
    if (testHistoryAfterRealtimeKeepsAscendingOrder() &&
        testAcceptedMessageReordersLocalPendingMessage() &&
