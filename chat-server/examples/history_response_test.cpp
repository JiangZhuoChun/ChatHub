...
bool testFinalHistoryResponseBody() {
    try {
        ...
        return object.at("request_id").as_string() == "request-1" &&
               object.at("is_last_chunk").as_bool() &&
               object.at("has_more").as_bool() &&
               response_messages.size() == 1 &&
               message.at("message_id").as_string() == "message-1" &&
               message.at("local_id").as_string() == "local-1" &&
               message.at("from").as_string() == "alice" &&
               message.at("to").as_string() == "bob" &&
...
bool testIntermediateAndEmptyHistoryResponseBody() {
    try {
        ...
        boost::json::array messages;
        messages.emplace_back(makeHistoryMessageObject(makeStoredMessage()));
        const auto intermediate = boost::json::parse(
            makeHistoryResultBody("request-2", messages, false, page_with_more)).as_object();

        repository::HistoryQueryResult empty_result;
        const auto empty = boost::json::parse(
            makeHistoryResultBody("request-3", boost::json::array{}, true, empty_result)).as_object();

        return !intermediate.if_contains("has_more") &&
               !intermediate.if_contains("next_cursor") &&
               !intermediate.at("is_last_chunk").as_bool() &&
               empty.at("messages").as_array().empty() &&
               empty.at("is_last_chunk").as_bool() &&
               !empty.at("has_more").as_bool() &&
...
int main() {
    const bool all_passed =
        ...
        runTest("intermediate and empty history response body",
                testIntermediateAndEmptyHistoryResponseBody()) &&
        runTest("serialized history response byte count",
                testSerializedHistoryResponseCanExceedBodyLimit());
